...
export default function PaginaGeradorEstampa() {
  ...
  const gerarImagens = async () => {
    ...
    const urls = await Promise.all([
      fetchImagem(prompt),
      fetchImagem(prompt + ' variação natural')
    ]);
    setImagens(urls);
  };

  const fetchImagem = async (prompt) => {
    return 'https://via.placeholder.com/1024x1024.png?text=Estampa+Gerada';
  };

  const gerarPSD = () => {
    alert('Exportação em PSD está em construção...');
  };
  ...
  const baixarEstampaAlta = (index) => {
    const link = document.createElement('a');
    link.href = imagens[index];
  ...
  return (
    <div className="pagina">
      ...
      <input placeholder="Estilo (ex: floral, onça...)" value={estilo} onChange={(e) => setEstilo(e.target.value)} />
